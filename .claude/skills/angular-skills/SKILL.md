...
Ported from `.github/skills/angular_skills/SKILL.md` (the GitHub Copilot version of this
skill) — keep both in sync when either changes.
...
Call `setTitle()` in every page component's `ngOnInit` (or constructor, for components without
an explicit `ngOnInit`):
